<?php
 ...
 if(!isset($_POST["polz_name"]) || !isset($_POST["polz_tel"]) || !isset($_POST["polz_massege"]))
 {
 ...
 $user_Massege = filter_var($_POST["polz_massege"], FILTER_SANITIZE_STRING);
 ...
 if(strlen($user_Massege)<3)
 {
 $otvet_serv = json_encode(array('text' => 'Поле Сообщение слишком короткое или пустое'));
 die($otvet_serv);
 }

 $message = "Имя: ".$user_Name.". Телефон: ".$user_Phone.". Сообщение: ".$user_Massege;
